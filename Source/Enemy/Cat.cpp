...
void Cat::Init() {
	...
	//テクスチャの読み込み
	mTexture.Load("Images/2dAction/Cat.png");
...
//準備時の処理
void Cat::_updateReserving() {

	mTimer += Time_I->GetDeltaTime();		//ジャンプまでの時間を計測
	...
	//1.秒経過したら状態をジャンプにし、タイマーをリセット
	if (mTimer >= 1.f) {
		mStatus = Status::Jumping;
		mTimer = 0.f;

		// ここで mDirection を明示的にコピーしてジャンプ中も保持させる（方向固定）
		mJumpDirection = mDirection;

		mVelocity.y = 500.f;
		mVelocity.x = (mJumpDirection == Direction::Right) ? 100.f : -100.f;
	}
}

//ジャンプ時の処理
void Cat::_updateJumping() {
	// 重力処理
	float gravityScale = 50.0f;
	float gravity = -9.8f * gravityScale; // 重力加速度（単位: ピクセル/秒^2）

	mVelocity.y += gravity * Time_I->GetDeltaTime(); // 加速度により速度を更新

	// 降下速度の最大値制限
	float maxFallSpeed = -1200.0f; // 単位: ピクセル/秒
	...
	//アニメーションの更新
	{
		...
		string playAnimName = animName_jump[(int)mJumpDirection];
		mSprite.PlayAnimation(playAnimName, AnimationPlayStyle::Continue);
	}

	// 位置更新
	mPosition += mVelocity * Time_I->GetDeltaTime(); // 速度に基づいて位置を更新
	_CheckRound();	//地面と接しているか確認
}

//プレイヤーと猫の二点間の距離を返す処理
float Cat::_GetDistanceBetweenTwo() {
	return mDistanceX;
}

//プレイヤーと猫の二点間の距離を測る処理
void Cat::_SetDistanceBetweenTwo() {
...
void Cat::_CheckRound() {
	...
	if (GetMap()->IsInsideWall(vCheckPos, CollisionWidth, CollisionHeight)) {
		...
		// 状態に応じて復帰
		if (mStatus == Status::Falling || mStatus == Status::Jumping) {
